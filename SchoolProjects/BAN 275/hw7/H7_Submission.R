future1=paste("Corresponding figures",".pdf")
# pdf(file=future1)
# homework 7
rm(list=ls())
library(quantmod)
...
adf.GDP <- adfTest(diff(GDP.ts,1,1),lag=1,type='ct') # ct: 12, 6, 1;
adf.GDP@test
...
adf.CPIAUCSL <- adfTest(diff(CPIAUCSL.ts,1,1),lag=11,type='c') #ct: 15; #c: 15, 11(good);
adf.CPIAUCSL@test
...
adf.DEXUSEU <- adfTest(diff(DEXUSEU.ts,1,1),lag=19,type='c') #ct: 19; #c: 19(good);
adf.DEXUSEU@test
...
adf.UNRATE <- adfTest(diff(UNRATE.ts,1,1),lag=2,type='nc') #ct: 15; #c: 15; #nc: 15, 2(good)
adf.UNRATE@test
...
adf.INDPRO <- adfTest(diff(INDPRO.ts,1,1),lag=2,type='c') #ct: 15; #c: 15,10,2;
adf.INDPRO@test
...
### First differencing
plot(diff(GE.ts,1,1))
adf.GE <- adfTest(diff(GE.ts,1,1),,lag=0,type='nc') #ct: 22; #c: 22; #nc: 22, 0;
adf.GE@test
