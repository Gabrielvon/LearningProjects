##----------------------------- 9 -----------------------------##
# Create empty matrix
alldata <- lapply(1:7, function(x){x<-matrix(NA,100,3)})
names(alldata) <- c("AIC", "BIC", "LogLik", "SE.E", "JB.P", "LB.P",'MAPE')
lapply(alldata,function(x){colnames(x)<-c('correct','over','under')})
attach(alldata)
...
#         plot(forc.un,main='forcasting underfit model')
        ...
        # aic
        alldata$AIC[i,]<-sapply(allfit,function(x){x$aic}) 
        # bic
        alldata$BIC[i,] <- sapply(allfit,function(x){x$bic})
        # Loglikhood
        alldata$LogLik[i,] <- sapply(allfit,function(x){x$loglik})
        ...
        alldata$SE.E[i,] <- sapply(allfit,function(x){
        ...
        # JarqueBera p value
        alldata$JB.P[i,] <- sapply(allfit,function(x){
                Resid <- x$residual[1:300]
                jarque.bera.test(Resid)$p.value
        }
        )
        # LjungBox p value
        alldata$LB.P[i,] <- sapply(allfit,function(x){
                Resid <- x$residual[1:300]
        ...
        # MAPE
        alldata$MAPE[i,] <- sapply(allfit, function(x){
